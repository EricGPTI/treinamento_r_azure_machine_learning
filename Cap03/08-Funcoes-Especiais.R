...
# do.call()
# Eecuta uma função em um objeto
# *** ATENÇÃO ***
# As funções da família apply aplicam uma função a cada elemento de um objeto(substitui um loop)
# A função do.call aplica uma função ao objeto interio e não a cada elemento individualmente
...
# Transformar a lista em matrzi, fazendo antes um unlist()
matrix(unlist(temp), ncol = 3, byrow = T)

names <- c("Brin, Sargey", "Page, Larry", "Dorsey, Jack", "Glass, Noah",
